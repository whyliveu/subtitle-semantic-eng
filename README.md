...
По умолчанию выдача делает 5 уникальных цитат из разных фильмов и старается покрыть разные десятилетия.
Если у вас всё ещё повторяется один фильм, увеличьте кандидатный пул:
`python -m app.search "откройся миру" --top-k 5 --record-type line_window --candidate-k 800`
Если хотите снять ограничение по периодам, добавьте `--no-diverse-periods`.
